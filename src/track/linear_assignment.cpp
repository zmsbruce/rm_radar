...
linear_assignment::matching_cascade(
    ...
    for (int level = 0; level < cascade_depth; level++) {
        ...
        TRACHER_MATCHD tmp = min_cost_matching(
            distance_metric, distance_metric_func, max_distance, tracks, robots,
...
linear_assignment::min_cost_matching(
    ...
    DYNAMICM cost_matrix = (distance_metric->*(distance_metric_func))(
        tracks, robots, track_indices, detection_indices);
...
DYNAMICM
linear_assignment::gate_cost_matrix(KalmanFilter *kf, DYNAMICM &cost_matrix,
    ...
    for (int i : detection_indices) {
        measurements.push_back(xyah(robots[i]));
    }
    for (size_t i = 0; i < track_indices.size(); i++) {

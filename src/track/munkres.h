...
class Munkres {
   ...
   public:
    ...
    static void minimize_along_direction(Matrix<Data> &matrix,
                                         const bool over_columns) {
        const size_t outer_size =
                         over_columns ? matrix.columns() : matrix.rows(),
                     inner_size =
                         over_columns ? matrix.rows() : matrix.columns();
   ...
   private:
    inline bool find_uncovered_in_matrix(const double item, size_t &row,
                                         size_t &col) const {
        const size_t rows = matrix.rows(), columns = matrix.columns();
            ...
            for (size_t col = 0; col < columns; col++) {
                if (0 == matrix(row, col)) {
                    for (size_t nrow = 0; nrow < row; nrow++)
                        if (STAR == mask_matrix(nrow, col)) goto next_column;
    ...
    int step3() {
        ...
        Main Zero Search
         ...
         1. Find an uncovered Z in the distance matrix and prime it. If no such
        zero exists, go to Step 5
    ...
    int step4() {
        ...
        // either a star or a prime that is part of the ``alternating
        // sequence``.
        ...
        Increment Set of Starred Zeros
         ...
         1. Construct the ``alternating sequence'' of primed and starred zeros:
            ...
            The sequence eventually terminates with an unpaired Z' = Z[2N] for
        some N.
        */
        bool madepair;
        do {
    ...
    int step5() {
        ...
        New Zero Manufactures
         ...
         1. Let h be the smallest uncovered entry in the (modified) distance
        matrix.

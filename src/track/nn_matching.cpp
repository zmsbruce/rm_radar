...
NearNeighborDisMetric::distance(const FEATURESS &features,
    ...
    DYNAMICM cost_matrix =
        Eigen::MatrixXf::Zero(targets.size(), features.rows());
    int idx = 0;
    for (int target : targets) {
        cost_matrix.row(idx) =
            (this->*_metric)(this->samples[target], features);
        idx++;
    }
    return cost_matrix;
}
    ...
    for (TRACKER_DATA &data : tid_feats) {
        ...
        if (samples.find(track_id) != samples.end()) {  // append
            ...
            if (newSize <= this->budget) {
                ...
                newSampleFeatures.block(0, 0, oldSize, k_feature_dim) =
                    samples[track_id];
                newSampleFeatures.block(oldSize, 0, addSize, k_feature_dim) =
                    newFeatOne;
                ...
                if (oldSize < this->budget) {  // original space is not enough;
                    ...
                    if (addSize >= this->budget) {
                        newSampleFeatures =
                            newFeatOne.block(0, 0, this->budget, k_feature_dim);
                    } else {
                        newSampleFeatures.block(0, 0, this->budget - addSize,
                                                k_feature_dim) =
                            samples[track_id]
                                .block(addSize - 1, 0, this->budget - addSize,
                                       k_feature_dim)
                                .eval();
                        newSampleFeatures.block(this->budget - addSize, 0,
                                                addSize, k_feature_dim) =
                            newFeatOne;
                    }
                    samples[track_id] = newSampleFeatures;
                    ...
                    if (addSize >= this->budget) {
                        ...
                        samples[track_id].block(0, 0, this->budget - addSize,
                                                k_feature_dim) =
                            samples[track_id]
                                .block(addSize - 1, 0, this->budget - addSize,
                                       k_feature_dim)
                                .eval();
                        samples[track_id].block(this->budget - addSize, 0,
                                                addSize, k_feature_dim) =
                            newFeatOne;
                    }
                }
